...
class MoonplaceScreen(Screens):
    ...
    def get_med_type(self, you):
        ...
        if you.status == "medicine fox apprentice" and not you.mentor:
            med_type = "you_app_mentorless"
        elif you.status == "medicine fox apprentice":
            med_type = "you_app_mentor"
        elif you.status == "medicine fox" and len(get_med_cats(Cat, working=False)) == 2:
            med_type = "two_meds"
        elif you.status == "medicine fox" and len(get_med_cats(Cat, working=False)) > 2:
            med_type = "multi_meds"
    ...
    def get_other_med_greeting(self, possible_texts):
        """ Handles other medicine fox greetings at the Moonplace """
        ...
        if "other_med" not in game.switches:
            ...
            for clan_name in game.clan.all_clans:
                ...
                for i in range(randint(1,3)):
                    if randint(1,4) != 1:
                        other_clan_meds.append(Name(status = "medicine fox"))
                    else:
                        other_clan_meds.append(Name(status = "medicine fox apprentice"))
            ...
            for i in range(len(game.switches["other_med_clan"])):
                for cat_name in game.switches["other_med"][i]:
                    if cat_name.status == "medicine fox apprentice":
                        if randint(1,2) == 1:
                            cat_name.status = "medicine fox"

            game.switches["other_med"] = self.randomly_remove_string(game.switches["other_med"])
            for clan_meds in game.switches["other_med"]:
                if len(clan_meds) == 0:
                    clan_meds.append(Name(status = "medicine fox apprentice"))
                if len(clan_meds) < 3:
                    if randint(1, 5) == 1:
                        clan_meds.append(Name(status = "medicine fox apprentice"))
