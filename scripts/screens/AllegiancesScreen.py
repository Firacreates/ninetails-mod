    ...
    def screen_switches(self):
        ...
        if game.clan.game_mode != 'classic':
            self.help_button = UIImageButton(scale(pygame.Rect(
                ...
                tool_tip_text="Your medicine foxes will gather herbs over each timeskip and during any patrols you send "
                              "them on. You can see what was gathered in the Log below! Your medicine foxes will give"
        ...
        if self.meds:
            ...
            if len(self.meds) >= 1 and number == 0:
                meds_cover = f"You have no medicine foxes who are able to work. Your Clan will be at a higher risk of death and disease."
            ...
            if herb_amount == 0:
                ...
                if len(self.meds) == 1:
                    med_concern = f"The medicine fox worries over the herb stores, they don't have nearly enough for the Clan."
                else:
                    med_concern = f"The medicine foxes worry over the herb stores, they don't have nearly enough for the Clan."
            elif 8 < herb_amount <= 20:
                med_concern = f"The herb stores are small, but it's enough for now."
            elif 20 < herb_amount <= 30:
                if len(self.meds) == 1:
                    med_concern = f"The medicine fox is content with how many herbs they have stocked up."
                else:
                    med_concern = f"The medicine foxes are content with how many herbs they have stocked up."
            elif 30 < herb_amount <= 50:
                if len(self.meds) == 1:
                    med_concern = f"The herb stores are overflowing and the medicine fox has little worry."
                else:
                    med_concern = f"The herb stores are overflowing and the medicine foxes have little worry."
            elif 50 < herb_amount:
                if len(self.meds) == 1:
                    med_concern = f"StarClan has blessed them with plentiful herbs and the medicine fox sends their thanks to Silverpelt."
                else:
                    med_concern = f"StarClan has blessed them with plentiful herbs and the medicine foxes send their thanks to Silverpelt."
            ...
            meds_cover = f"You have no medicine foxes, your clan will be at higher risk of death and sickness."
