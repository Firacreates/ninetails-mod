...
class Cat():
    ...
    ages = [
        'newborn', 'cub', 'adolescent', 'young adult', 'adult', 'senior adult',
        'senior'
    ]
    age_moons = {
    ...
    def __init__(self,
        ...
        if not loading_cat:
            ...
            if self.gender == "female" and not self.status in ['newborn', 'cub']:
                if trans_chance == 1:
            ...
            elif self.gender == "male" and not self.status in ['newborn', 'cub']:
                if trans_chance == 1:
...
# pylint: disable=f-string-without-interpolation
    ...
    def status_change(self, new_status, resort=False):
        """ Changes the status of a cat. Additional functions are needed if you want to make a cat a leader or deputy.
            new_status = The new status of a cat. Can be 'apprentice', 'medicine fox apprentice', 'warrior'
                        'medicine fox', 'elder'.
        ...
        elif self.status == 'medicine fox apprentice':
            pass
    ...
    def rank_change_traits_skill(self, mentor):
        ...
        if self.status in ["warrior", "medicine fox", "mediator", "queen"]:
    ...
    def one_moon(self):
        ...
        if self.moons == 1 and self.status == "newborn":
            self.status = 'cub'
        elif self.moons == 0 and self.status == "cub":
            self.status = 'newborn'
        self.in_camp = 1
        ...
        if self.status in ['apprentice', 'mediator apprentice', 'medicine fox apprentice', "queen's apprentice"]:
            self.update_mentor()
    ...
    def retire_cat(self):
        ...
        if self.moons > 6 and self.status in ["apprentice", "medicine fox apprentice", "mediator apprentice", "queen's apprentice"]:
    ...
    def is_valid_mentor(self, potential_mentor: Cat):
        ...
        if potential_mentor.dead or potential_mentor.outside:
            return False
        # Match jobs
        if self.status == 'medicine fox apprentice' and potential_mentor.status != 'medicine fox':
            return False
    ...
    def is_dateable(self,
        ...
        # check age. allow adolescents to flirt with each other. prevent cubs and newborns from flirting
        ...
        if age_restriction:
            if (self.moons < 12 or other_cat.moons < 12):
                if self.age in ["adolescent"] or other_cat.age in ["adolescent"]:
                    if self.age != other_cat.age:
                        return False
            if self.age in ["newborn", "cub"] or other_cat.age in ["newborn", "cub"]:
                return False
...
def create_example_cats():
    ...
    for a in range(12):
        if a in e:
            game.choose_cats[a] = Cat(status='cub', biome=None)
        else:
