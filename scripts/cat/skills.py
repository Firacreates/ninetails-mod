    ...
    def progress_skill(self, the_cat):
        ...
        if not self.primary:
            ...
            if parental_paths and random.randint(0, 1):
                self.primary = Skill(random.choice(parental_paths), points=0, interest_only=True if the_cat.status in ["apprentice", "cub"] else False)
            else:
                self.primary = Skill.get_random_skill(points=0, interest_only=True if the_cat.status in ["apprentice", "cub"] else False)
        
        
        if not (the_cat.outside or the_cat.exiled):
            if the_cat.status == 'cub':
            ...
            if the_cat.age not in ["cub", "adolescent"]:
                self.primary.interest_only = False
                if self.secondary:
                    self.secondary.interest_only = False
