...
class Events:
    ...
    def one_moon(self):
        ...
        if game.clan.your_cat.status == "medicine fox apprentice" or game.clan.your_cat.status == "medicine fox":
            game.switches["attended half-moon"] = False
        
        if any(
                str(cat.status) in {
                    'leader', 'deputy', 'warrior', 'medicine fox',
                    'medicine fox apprentice', 'apprentice', 'mediator',
        ...
        for cat in Cat.all_cats.copy().values():
            if cat.shunned == 2:
                ...
                elif cat.status in ["medicine fox", "medicine fox apprentice"]:
                    string = f"{cat.name} has been forced to step down as a medicine fox due to their crimes."
                ...
                elif cat.status in ["warrior", "apprentice", "cub"]:
        ...
        if game.clan.game_mode in ["expanded", "cruel season"]:
            ...
            has_med = any(
                str(cat.status) in {"medicine fox", "medicine fox apprentice"}
                and not cat.dead and not cat.outside and not cat.shunned
                for cat in Cat.all_cats.values())
            if not has_med:
                string = f"{game.clan.name}Clan has no medicine fox!"
                game.cur_events_list.insert(0, Single_Event(string, "alert"))
        ...
        if not game.clan.your_cat.dead and game.clan.your_cat.status != 'exiled' and not game.clan.your_cat.outside:
            ...
            elif game.clan.your_cat.status in ['apprentice', 'medicine fox apprentice', 'mediator apprentice', "queen's apprentice"]:
                self.generate_events()
            elif game.clan.your_cat.status in ['warrior', 'medicine fox', 'mediator', "queen"] and not game.clan.your_cat.w_done and game.clan.your_cat.shunned == 0:
    ...
    def gain_acc(self):
        if game.clan.clan_settings["all accessories"]:
            return
        possible_accs = ["WILD", "PLANT", "COLLAR", "FLOWER", "PLANT2", "SNAKE", "SMALLANIMAL", "DEADINSECT", "ALIVEINSECT", "FRUIT", "CRAFTED", "TAIL2","Yokai"]
        acc_list = []
        ...
        if "Yokai" in possible_accs:
            acc_list.extend(Pelt.yokai_accessories)
    ...
    def check_achievements(self):
        ...
        for cat in clan_cats:
            ...
            if Cat.all_cats.get(cat).status == 'apprentice' and Cat.all_cats.get(cat).name.prefix == "Pea" and Cat.all_cats.get(cat).pelt.white_colours:
                achievements.add("33")
            if Cat.all_cats.get(cat).status == 'cub' and Cat.all_cats.get(cat).moons > 5:
                achievements.add("34")
    ...
    def generate_events(self):
        ...
        if game.clan.your_cat.status not in ["newborn", "cub"] and game.clan.your_cat.shunned == 0 and not game.clan.your_cat.dead:
    ...
    def generate_ceremony(self):
        if game.clan.your_cat.former_mentor:
            if Cat.all_cats[game.clan.your_cat.former_mentor[-1]].dead and game.clan.your_cat.status == 'medicine fox':
    ...
    def generate_death_event(self):
        if game.clan.your_cat.status == 'cub':
            ceremony_txt = random.choice(self.b_txt['death_kit'])
            game.cur_events_list.insert(1, Single_Event(ceremony_txt))
        elif game.clan.your_cat.status == 'medicine fox apprentice':
    ...
    def generate_df_events(self):
        ...
        if random.randint(1,30) == 1:
            ...
            counter = 0
            while r_clanmate.dead or r_clanmate.outside or r_clanmate.status in ['cub', 'newborn', 'deputy', 'leader'] or r_clanmate.joined_df or r_clanmate.ID == game.clan.your_cat.ID:
                counter+=1
                if counter > 15:
                    return
                r_clanmate = Cat.all_cats.get(random.choice(game.clan.clan_cats))
            ...
            if cat.status in _ and \
                ...
                game.cur_events_list.append(
                    Single_Event(
                        ...
                        f"become the Clan's newest medicine fox. ", "ceremony",
                        cat.ID))
                cat.status_change("medicine fox")
    ...
    def herb_destruction(self):
        ...
        if len(game.clan.herbs.keys()) >= 10 and not int(
            ...
            if not game.clan.herbs:
                ...
                possible_events = [
                    # pylint: disable=line-too-long
                    f"The {chosen_ally.name}Clan medicine fox comes asking if your Clan has any herbs to spare. "
                    f"Unfortunately, your stocks are bare, and you are unable to provide any help. ",
                    f"A medicine fox from {chosen_ally.name}Clan comes comes to your Clan, asking for herbs "
                ...
                if game.clan.herbs[herb_given] > 2:
                    ...
                    possible_events = [
                        f"The {chosen_ally.name}Clan medicine fox comes asking if your Clan has any {herb_given.replace('_', ' ')} to spare. "  # pylint: disable=line-too-long
                        f"Graciously, your Clan decides to aid their allies and share the herbs.",
                        f"The medicine fox apprentice from {chosen_ally.name}Clan comes asking for {herb_given.replace('_', ' ')}. "  # pylint: disable=line-too-long
                    ...
                    if herb_given == 'lungwort':
                        possible_events.extend([
                            f"The {chosen_ally.name}Clan medicine fox apprentice comes to your camp, pleading for help "  # pylint: disable=line-too-long
                            f"with a yellowcough epidemic. Your Clan provides the cat with some of their extra lungwort.",
                            # pylint: disable=line-too-long
                            f"A medicine fox from {chosen_ally.name}Clan comes to your Clan, asking for lungwort to heal a "  # pylint: disable=line-too-long
                    ...
                    possible_events = [
                        ...
                        # pylint: disable=line-too-long
                        f"The medicine fox apprentice from {chosen_ally.name}Clan comes asking for herbs. They refuse to "  # pylint: disable=line-too-long
                        f"say why their Clan needs them and your Clan decides not to share their precious few {herb_given.replace('_', ' ')}."
                        # pylint: disable=line-too-long
                    ]
                    if herb_given == 'lungwort':
                        possible_events.extend([
                            f"The {chosen_ally.name}Clan medicine fox apprentice comes to your camp, pleading for help with"  # pylint: disable=line-too-long
                            f" a yellowcough epidemic. Your Clan can't spare the precious herb however, and turns them away.",
                            # pylint: disable=line-too-long
                            f"A medicine fox from {chosen_ally.name}Clan comes to your Clan, asking for lungwort to heal "  # pylint: disable=line-too-long
    ...
    def handle_focus(self):
        ...
        if game.clan.clan_settings.get("business as usual") or game.clan.clan_settings.get("rest and recover"):
            ...
            healthy_meds = list(filter(
                lambda c: c.status == "medicine fox" and not c.dead
                and not c.outside and not c.exiled and not c.not_working(),
                Cat.all_cats.values()
            ))
            ...
            # handle medicine fox apprentices
            healthy_med_apps = list(filter(
                lambda c: c.status == "medicine fox apprentice" and not c.dead
                and not c.outside and not c.exiled and not c.not_working(),
                Cat.all_cats.values()
            ))
    ...
    def handle_lost_cats_return(self):
        ...
        if lost_cat.ID == game.clan.your_cat.ID:
            if lost_cat.status == "cub" and lost_cat.moons > 4:
        ...
        for x in [lost_cat] + [Cat.fetch_cat(i) for i in additional_cats]:             
            ...
            if x.status in ["apprentice", "medicine fox apprentice", "mediator apprentice", "queen's apprentice", "cub", "newborn"]:
                if x.moons >= 15:
                    if x.status == "medicine fox apprentice":
                        self.ceremony(x, "medicine fox")
                ...
                elif x.status in ["cub", "newborn"] and x.moons >= 6:
                    self.ceremony(x, "apprentice")
            elif x.status != 'medicine fox':
                if x.moons == 0:
                    x.status = 'newborn'
                elif x.moons < 6:
                    x.status = "cub"
    ...
    def one_moon_cat(self, cat):
        ...
        elif cat.ID != game.clan.your_cat.ID and cat.status not in ['cub', 'elder', 'newborn'] and not cat.outside and not cat.dead:
    ...
    def perform_ceremonies(self, cat):
        ...
        if not cat_dead:
            ...
            if cat.status == 'medicine fox' and game.clan.medicine_cat is None:
            ...
            # apprentice a cub to either med or warrior
            if cat.moons == cat_class.age_moons["adolescent"][0]:
                if cat.status == 'cub':
                    ...
                    med_cat_list = [i for i in Cat.all_cats_list if
                                    i.status in ["medicine fox", "medicine fox apprentice"] and not (
                                            i.dead or i.outside)]

                    # check if the medicine fox is an elder
                    has_elder_med = [c for c in med_cat_list if c.age == 'senior' and c.status == "medicine fox"]

                    very_old_med = [
                        c for c in med_cat_list
                        if c.moons >= 150 and c.status == "medicine fox"
                    ]
                    ...
                    has_med_app = any(cat.status == "medicine fox apprentice"
                    ...
                    # Clan has at least one non-elder medicine fox.
                    elif not has_med:
                    ...
                    if not has_med_app and not int(random.random() * chance):
                        self.ceremony(cat, 'medicine fox apprentice')
                        self.ceremony_accessory = True
                        self.gain_accessories(cat)
            ...
            if cat.status in [
                ...
                "medicine fox apprentice", "queen's apprentice"
            ] and cat.shunned == 0:
                ...
                if _ready:
                    ...
                    # promote to med cat
                    elif cat.status == 'medicine fox apprentice':
                        self.ceremony(cat, 'medicine fox', preparedness)
                        self.ceremony_accessory = True
                        self.gain_accessories(cat)
    ...
    def ceremony(self, cat, promoted_to, preparedness="prepared"):
        ...
        try:
            ...
            if cat.shunned == 0 and cat.forgiven > 0 and cat.forgiven < 5 and cat.status in ["apprentice", "medicine fox apprentice", "mediator apprentice", "queen's apprentice"]:
                possible_ceremonies = possible_ceremonies.intersection(self.ceremony_id_by_tag["forgiven"])

            # Get ones for prepared status ----------------------------------------------
            if promoted_to in ["warrior", "medicine fox", "mediator", "queen"]:
        ...
        random_honor = None
        if promoted_to in ['warrior', 'mediator', 'medicine fox', "queen"]:
            resource_dir = "resources/dicts/events/ceremonies/"
            with open(f"{resource_dir}ceremony_traits.json",
                    encoding="ascii") as read_file:
                TRAITS = ujson.loads(read_file.read())
        ...
        if cat.status in ["warrior", "medicine fox", "mediator", "queen"]:
    ...
    def gain_accessories(self, cat):
        ...
        if cat.status in ['medicine fox', 'medicine fox apprentice']:
    ...
    def handle_apprentice_EX(self, cat):
        ...
        if cat.status in [
            "apprentice", "medicine fox apprentice", "mediator apprentice", "queen's apprentice"
        ]:
            ...
            if cat.status == "medicine fox apprentice":
    ...
    def invite_new_cats(self, cat):
        ...
        if possible_other_cats:
            ...
            if cat.status in ["apprentice", "medicine fox apprentice"
        ...
        if not int(random.random() * chance) and \
                cat.age != 'cub' and cat.age != 'adolescent' and not self.new_cat_invited:
    ...
    def handle_injuries_or_general_death(self, cat):
        ...
        try:
            if cat.status == "cub" or cat.status == "newborn":
                num_queens = 0
            ...
            if not int(random.random() * chance):  # 1/chance to infect
                ...
                if illness == 'cubcough':
                    ...
                    alive_cats = list(
                        filter(
                            lambda kitty:
                            (kitty.status in ['cub', 'newborn'] and not kitty.dead and
                             not kitty.outside), Cat.all_cats.values()))
                    alive_count = len(alive_cats)
    ...
    def exile_or_forgive(self, cat):
        ...
        if cat.shunned > 2:
            ...
            if fate != 1:
                ...
                if cat.status in ['medicine fox', 'deputy', 'mediator', 'queen']:
                    ...
                    if cat.status in ["apprentice", "medicine fox apprentice", "mediator apprentice", "queen's apprentice"]:
                        if cat.moons >= 15:
                            if cat.status == "medicine fox apprentice":
                                self.ceremony(cat, "medicine fox")
                            elif cat.status == "mediator apprentice":
                    ...
                    elif cat.status in ["cub", "newborn"] and cat.moons >= 6:
                        self.ceremony(cat, "apprentice") 
