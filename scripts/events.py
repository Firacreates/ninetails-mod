...
class Events:
    ...
    def one_moon(self):
        ...
        if game.clan.game_mode in ["expanded", "cruel season"]:
            ...
            if not med_fullfilled:
                string = f"{game.clan.name}Clan does not have enough healthy medicine foxes! Cats will be sick/hurt " \
    ...
    def handle_lost_cats_return(self):
        ...
        for x in [lost_cat] + [Cat.fetch_cat(i) for i in additional_cats]:             
            ...
            if x.status in ["apprentice", "medicine fox apprentice", "mediator apprentice", "cub", "newborn"]: 
                if x.moons >= 15:
    ...
    def handle_fading(self, cat):
        ...
        if game.clan.clan_settings["fading"] and not cat.prevent_fading \
            ...
            if cat.dead_for > age_to_fade:
                ...
                if game.clan.medicine_fox:
                    if game.clan.medicine_fox.ID == cat.ID:
                        if game.clan.med_cat_list:  # If there are other med cats
                            game.clan.medicine_fox = Cat.fetch_cat(
                                game.clan.med_cat_list[0])
                        else:
                            game.clan.medicine_fox = None
    ...
    def check_war(self):
        ...
        if not game.clan.leader or not game.clan.deputy or not game.clan.medicine_fox:
            for event in war_events:
                ...
                if not game.clan.deputy and "dep_name" in event:
                    war_events.remove(event)
                if not game.clan.medicine_fox and "med_name" in event:
                    war_events.remove(event)
    ...
    def perform_ceremonies(self, cat):
        ...
        if not cat_dead:
            ...
            if cat.status == 'medicine fox' and game.clan.medicine_fox is None:
                game.clan.medicine_fox = cat
            ...
            if cat.moons == cat_class.age_moons["adolescent"][0]:
                if cat.status == 'cub':
                    ...
                    if has_elder_med == med_cat_list:
                        # These chances apply if all the current medicine foxes are elders.
                        if has_med:
                            chance = int(chance / 2.22)
                        else:
                            chance = int(chance / 13.67)
                    elif very_old_med == med_cat_list:
                        # These chances apply is all the current medicine foxes are very old.
                        if has_med:
                            chance = int(chance / 3)
                        else:
    ...
    def ceremony(self, cat, promoted_to, preparedness="prepared"):
        ...
        try:
            ...
            if promoted_to in ["warrior", "medicine fox", "mediator"]:
        ...
        random_honor = None
        if promoted_to in ['warrior', 'mediator', 'medicine fox']:
            resource_dir = "resources/dicts/events/ceremonies/"
        ...
        if cat.status in ["warrior", "medicine fox", "mediator"]:
    ...
    def handle_apprentice_EX(self, cat):
        ...
        """
        if cat.status in [
            "apprentice", "medicine fox apprentice", "mediator apprentice"
        ]:
