...
class Clan():
    ...
    def __init__(self,
        ...
        self.medicine_fox = medicine_fox
        self.med_cat_list = []
        self.med_cat_predecessors = 0
        
        self.med_cat_number = len(
            self.med_cat_list
        )  # Must do this after the medicine fox is added to the list.
        self.herbs = {}
        self.age = 0
    ...
    def post_initialization_functions(self):
        ...
        if self.medicine_fox is not None:
            self.clan_cats.append(self.medicine_fox.ID)
            self.med_cat_list.append(self.medicine_fox.ID)
            if self.medicine_fox.status != 'medicine fox':
                Cat.all_cats[self.medicine_fox.ID].status_change('medicine fox')
    def create_clan(self):
        ...
        the program starts
        """
        self.instructor = Cat(status=choice(["apprentice", "mediator apprentice", "medicine fox apprentice", "warrior",
                                             "medicine fox", "leader", "mediator", "deputy", "elder"]),
                              )
        ...
        for i in key_copy:  # Going through all currently existing cats
            ...
            if Cat.all_cats[i] != self.leader and Cat.all_cats[i] != \
                    self.medicine_fox and Cat.all_cats[i] != \
    ...
    def __repr__(self):
        if self.name is not None:
            _ = f'{self.name}: led by {self.leader.name}' \
                f'with {self.medicine_fox.name} as med. cat'
            return _
    ...
    def new_medicine_cat(self, medicine_fox):
        """
        TODO: DOCS
        """
        if medicine_fox:
            if medicine_fox.status != 'medicine fox':
                Cat.all_cats[medicine_fox.ID].status_change('medicine fox')
            if medicine_fox.ID not in self.med_cat_list:
                self.med_cat_list.append(medicine_fox.ID)
            medicine_fox = self.med_cat_list[0]
            self.medicine_fox = Cat.all_cats[medicine_fox]
            self.med_cat_number = len(self.med_cat_list)

    def remove_med_cat(self, medicine_fox):
        """
        Removes a med cat. Use when retiring, or switching to warrior
        """
        if medicine_fox:
            if medicine_fox.ID in game.clan.med_cat_list:
                game.clan.med_cat_list.remove(medicine_fox.ID)
                game.clan.med_cat_number = len(game.clan.med_cat_list)
            if self.medicine_fox:
                if medicine_fox.ID == self.medicine_fox.ID:
    ...
    def save_clan(self):
        ...
        # MED CAT DATA
        if self.medicine_fox:
            clan_data["med_cat"] = self.medicine_fox.ID
        else:
