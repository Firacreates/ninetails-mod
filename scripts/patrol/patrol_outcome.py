...
class PatrolOutcome():
    ...
    def _get_stat_cat(self, patrol: 'Patrol') -> bool:
        ...
        for kitty in patrol.patrol_cats:
            ...
            if "app" in self.can_have_stat \
                    and kitty.status not in ['apprentice', "medicine fox apprentice"]:
                continue
            
            if "adult" in self.can_have_stat and kitty.status in ['apprentice', "medicine fox apprentice"]:
                continue
            
            if "healer" in self.can_have_stat and kitty.status not in ["medicine fox", "medicine fox apprentice"]:
                continue
    ...
    def __create_new_cat_block(self, i:int, attribute_list: List[str], patrol:'Patrol') -> List[Cat]: 
        ...
        for tag in attribute_list:
            ...
            for index in mate_indexes:
                if index in in_patrol_cats:
                    if in_patrol_cats[index] in ("apprentice", "medicine fox apprentice"):
        ...
        for _tag in attribute_list:
            ...
            if match.group(1) in ("newborn", "cub", "elder", "apprentice", "warrior", 
                                  "mediator apprentice", "mediator", "medicine fox apprentice", 
                                  "medicine fox", "deputy", "leader"):
        ...
        # gives a random status if none was specified in the patrol. cub cannot be chosen randomly
            ...
            if age:
                ...
                if age < 1:
                    ...
                    status = choice(["apprentice", "apprentice", "apprentice", "mediator apprentice", "medicine fox apprentice", "queen's apprentice"])
                elif age < 119:
                    status = choice(["warrior", "warrior", "medicine fox", "mediator", "queen", "warrior", "warrior", "medicine fox", "medicine fox", "mediator", "deputy", "leader"])
                else:
        ...
        # gives a random status if none was specified in the patrol. cub cannot be chosen randomly
            if status is None:
                status = choice(["elder", "elder", "elder", "elder", "elder", "apprentice", "warrior", "warrior", "warrior", "warrior", "warrior", "warrior", "mediator apprentice", "mediator", "mediator", "medicine fox apprentice", "medicine fox", "medicine fox", "medicine fox", "medicine fox", "queen's apprentice", "queen", "queen", "queen", "queen","leader"])

        #and age, dependant on status
            if status in "cub":
                age = randint(1,5)
            elif status in ["apprentice", "mediator apprentice", "medicine fox apprentice", "queen's apprentice"]:
                age = randint (6,11)
            elif status in ["warrior", "medicine fox", "mediator", "queen"]:
                age = randint (12, 119)
        ...
        new_cats = create_new_cat(Cat,
                                ...
                                kit=False if litter else status in ["cub", "newborn"],  # this is for singular kits, litters need this to be false
                                litter=litter,
