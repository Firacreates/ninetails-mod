...
class Patrol():
    ...
    def add_patrol_cats(self, patrol_cats: List[Cat], clan: Clan) -> None:
        ...
        for cat in patrol_cats:
            ...
            if cat.status == 'apprentice' or cat.status == 'medicine fox apprentice':
        ...
        if "medicine fox" in self.patrol_status_list:
            index = self.patrol_status_list.index("medicine fox")
            self.patrol_leader = self.patrol_cats[index]
        # If there is no medicine fox, but there is a medicine fox apprentice, set them as the patrol leader.
        # This prevents warrior from being treated as medicine cats in medicine fox patrols.
        elif "medicine fox apprentice" in self.patrol_status_list:
            index = self.patrol_status_list.index("medicine fox apprentice")
            self.patrol_leader = self.patrol_cats[index]
        ...
        # sets leader as patrol leader
            ...
            possible_leader = [i for i in self.patrol_cats if i.status not in 
                               ["medicine fox apprentice", "apprentice"]]
            if possible_leader:
    ...
    def get_possible_patrols(self, current_season:str, biome:str, camp:str, patrol_type:str,
        ...
        patrol_type = "med" if ['medicine fox', 'medicine fox apprentice'] in self.patrol_status_list else patrol_type
        ...
        for patrol in possible_patrols:
            ...
            if game.current_screen == 'patrol screen4':
                if "you_med" in patrol.tags:
                    if game.clan.your_cat.status != 'medicine fox':
                        continue
