...
def get_alive_clan_queens(living_cats):
    living_kits = [cat for cat in living_cats if not (cat.dead or cat.outside) and cat.status in ["cub", "newborn"]]
...
def get_alive_kits(Cat):
    """
    returns a list of IDs for all living cubs in the clan
    """
    alive_kits = [i for i in Cat.all_cats.values() if
                  i.age in ['cub', 'newborn'] and not i.dead and not i.outside]
...
def get_alive_meds(Cat):
    ...
    """
    alive_apps = [i for i in Cat.all_cats.values() if
                  (i.status == 'medicine fox' or i.status == 'medicine fox apprentice') and not i.dead and not i.outside]
...
def get_med_cats(Cat, working=True):
    ...
    possible_med_cats = [i for i in all_cats if
                         i.status in ['medicine fox apprentice', 'medicine fox'] and not (i.dead or i.outside)]
...
def create_new_cat(Cat,
    ...
    :param kit: set True if the cat is a lone cub - default: False
    :param litter: set True if a litter of cubs needs to be generated - default: False
    ...
    :param age: set the age of the new cat(s) - default: None (will be random or if kit/litter is true, will be cub.
    ...
    if not isinstance(age, int):
        if status == "newborn":
            ...
            age = randint(1, 5)
        elif status in ('apprentice', 'medicine fox apprentice', 'mediator apprentice'):
            age = randint(6, 11)
        elif status in ('apprentice', 'medicine fox apprentice', 'mediator apprentice') and litter:
            age = randint(20, 30)
        elif status == 'warrior':
            age = randint(23, 120)
        elif status == 'medicine fox':
            age = randint(23, 140)
        elif status == 'elder':
            age = randint(120, 130)
        else:
                ...
                if scarchance == 1 or 2 or 3:
                    ...
                    if new_cat.status in ["warrior", "deputy", "leader"]:
                        ...
                        if scarchance == 1:
                            ...
                            new_cat.pelt.scars.append(scar)
                    elif new_cat.status in ["medicine fox", "apprentice", 
                    "elder", "medicine fox apprentice", "queen", "mediator", 
        ...
        for i in cat.pelt.accessories:
            ...
            if not acc_hidden:
                try:
                    ...
                    elif i in cat.pelt.yokai_accessories:
                        new_sprite.blit(sprites.sprites['acc_yokai' + i + cat_sprite], (0, 0))
                except:
                    continue
